...
| `LBL_REGISTER_SSO_PAGE_TITLE` | SSO 注册 | SSO で登録 | SSO Registration | SSO 页面标题 |
...
| `BTN_REGISTER_SSO_LINK` | SSO 注册 | SSO で登録 | Register with SSO | SSO 入口链接 |
| `BTN_REGISTER_FREE_LINK` | 普通注册 | 通常の登録 | Standard Registration | 自由注册入口 |

### 4.3 文案类 (TXT_)
| Key | 中文(zh) | 日文(ja) | 英文(en) | 使用位置 |
|---|---|---|---|---|
| `TXT_REGISTER_EYEBROW` | 账户注册 | アカウント登録 | Account Registration | Eyebrow 小字 |
| `TXT_REGISTER_SSO_EYEBROW` | 单点登录 | シングルサインオン | Single Sign-On | SSO Eyebrow |
...
| `TXT_REGISTER_TERMS` | 我同意 {{termsLink}} 和 {{privacyLink}} | {{termsLink}} および {{privacyLink}} に同意します | I agree to {{termsLink}} and {{privacyLink}} | 条款勾选 |
| `TXT_REGISTER_TERMS_LINK` | 服务条款 | 利用規約 | Terms of Service | 条款链接文本 |
| `TXT_REGISTER_PRIVACY_LINK` | 隐私政策 | プライバシーポリシー | Privacy Policy | 隐私链接文本 |
...
- **占位符列表**：
  ...
  |--------|------|--------|
  | `{{seconds}}` | 限流等待秒数 | 60 |
  | `{{termsLink}}` | 条款链接 HTML | `<a href="/terms">利用規約</a>` |
  | `{{privacyLink}}` | 隐私链接 HTML | `<a href="/privacy">プライバシーポリシー</a>` |
